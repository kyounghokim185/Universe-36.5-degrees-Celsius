...
export default function VeoStudio({ userData, countryData, languageData, partyOptions }) {
                ...
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                    ...
                    <div className="w-full md:w-1/3 bg-stone-50 p-6 border-r border-stone-100 overflow-y-auto font-mono text-xs md:text-sm max-h-[150px] md:max-h-full border-b md:border-b-0 border-stone-200">
                        ...
                        <div className="space-y-3">
                            ...
                            {status === 'prompting' && <div className="text-orange-500 animate-pulse">Optimizing prompt...</div>}

                            {status === 'imagining' && (
                                <div className="text-orange-500 animate-pulse">
                                    Generating 3 style drafts...
                                </div>
                            )}

                            {generatedImages.length > 0 && <div className="text-stone-600">[Done] Drafts generated.</div>}

                            {status === 'selection' && <div className="text-blue-500 animate-pulse">Waiting for user selection...</div>}

                            {status === 'converting' && (
                                <div className="text-purple-600 bg-purple-50 p-2 rounded">
                                    <div className="mb-1 font-bold">VEO 3 Rendering...</div>
                                    <div>Selection: Style #{selectedImageIndex + 1}</div>
                                    <div>Request: "{customRequest || 'None'}"</div>
                                </div>
                            )}
                            {status === 'ready' && <div className="text-green-600 font-bold">[Complete] Video is ready!</div>}
                        </div>
                    ...
                    <div className="w-full md:w-2/3 relative bg-white flex flex-col items-center p-4 md:p-8 gap-6 overflow-y-auto flex-1">
                        ...
                        {(status === 'prompting' || status === 'imagining') && (
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div className="text-center space-y-2">
                                    <h3 className="text-2xl font-serif font-bold text-gray-800">추억을 그리는 중...</h3>
                                    <p className="text-stone-500">당신의 이야기를 바탕으로<br />3가지 파티 분위기를 만들고 있습니다.</p>
                                </div>
                            </div>
                        )}
                        ...
                        {status === 'selection' && (
                            <div className="w-full h-full flex flex-col">
                                ...
                                <p className="text-stone-500 text-center text-sm mb-8">가장 마음에 드는 사진을 선택해주세요.</p>
                        ...
                        {status === 'converting' && (
                            <div className="flex flex-col items-center w-full max-w-md space-y-6">
                                <div className="text-orange-500 font-serif font-bold text-2xl animate-pulse">VEO 3 Generating...</div>
                                ...
                                <div className="text-center">
                                    <p className="text-gray-600 font-medium mb-1">시간을 되돌리고 있습니다...</p>
                                    <p className="text-stone-400 text-xs">최고 화질로 렌더링 중입니다.</p>
                                </div>
                            </div>
                        )}
